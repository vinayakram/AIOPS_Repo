...
$markdownPath = Join-Path $root "REMEDIATION_POC_BILINGUAL_DECK.md"
$output = Join-Path $root "Remediation-POC-Bilingual-Deck.pptx"
...
$slides = Parse-Slides -Lines $lines
...
try {
    foreach ($slideDef in $slides) {
        ...
        $subtitleBox.TextFrame.TextRange.Text = $slideDef.Subtitle
        ...
        $footer = $slide.Shapes.AddTextbox(1, 42, 500, 860, 20)
        $footer.TextFrame.TextRange.Text = "Remediation POC - Bilingual deck"
