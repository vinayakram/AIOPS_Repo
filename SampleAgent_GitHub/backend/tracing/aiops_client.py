"""
Non-blocking AIops Telemetry client for the SampleAgent GitHub backend that forwards
completed pipeline traces to the AIops ingest API. Builds structured span payloads from
TraceContext step timers and dispatches them in daemon background threads to avoid blocking
the main request path.

SampleAgent GitHubバックエンドのノンブロッキングAIopsテレメトリクライアント。完了したパイプライン
トレースをAIOpsインジェストAPIに転送する。TraceContextのステップタイマーから構造化スパン
ペイロードを構築し、メインリクエストパスをブロックしないよう、デーモンバックグラウンドスレッド
で送信する。
"""
import uuid
import logging
