"""
SQLAlchemy database models and session management for the SampleAgent GitHub backend using SQLite.
Defines the User table for authentication and the TraceLog table for persisting per-query
pipeline execution metadata. Includes database initialisation and default admin user seeding.

SQLiteを使用するSampleAgent GitHubバックエンドのSQLAlchemyデータベースモデルおよびセッション管理。
