...
PROMETHEUS_URL="${PROMETHEUS_URL:-http://10.169.82.55:9090}"
GRAFANA_URL="${GRAFANA_URL:-http://10.169.82.55:3000/d/node_exporter_91/node-exporter-91?orgId=1&from=now-24h&to=now&timezone=browser&var-instance=&refresh=1m}"
CONTAINER_NAME="${CONTAINER_NAME:-medical-rag-pod}"
DISPLAY_CONTAINER_NAME="${DISPLAY_CONTAINER_NAME:-sample-agent-pod}"
CADVISOR_INSTANCE="${CADVISOR_INSTANCE:-10.169.91.16:8080}"
